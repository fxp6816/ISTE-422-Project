...
public class EdgeConvertFileParser {
   ...
   public EdgeConvertFileParser(File constructorFile) {
      //logger.info("Enter EdgeConvertFileParser");

      //logger.debug("EdgeConvertFileParser Instantiated...");
   ...
   public void openFile(File inputFile) {
      try {
         ...
         if (currentLine.startsWith(EDGE_ID)) { //the file chosen is an Edge Diagrammer file
            ...
            ParseEdge parseEdge = new ParseEdge(inputFile,br);
            parseEdge.parse();

            ArrayList combineFields = new ArrayList();
            ArrayList combineTables = new ArrayList();
            combineFields.addAll(alFields);
            combineFields.addAll(parseEdge.getAlFields());
            alFields = combineFields;
            combineTables.addAll(parseEdge.getAlTables());
            combineTables.addAll(parseEdge.getAlTables());
            alTables = combineTables;
            ...
            if (currentLine.startsWith(SAVE_ID)) { //the file chosen is a Save file created by this application
               ...
               ParseSave parseSave = new ParseSave(br);
               parseSave.parse();

               ArrayList combineFields = new ArrayList();
               ArrayList combineTables = new ArrayList();
               combineFields.addAll(alFields);
               combineFields.addAll(parseSave.getAlFields());
               alFields = combineFields;
               combineTables.addAll(alTables);
               combineTables.addAll(parseSave.getAlTables());
               alTables = combineTables;
         ...
         //logger.debug("Failed opening File");
