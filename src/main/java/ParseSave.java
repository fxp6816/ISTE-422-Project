...
public class ParseSave implements Parser {
    private StringTokenizer stTables, stNatFields, stRelFields, stField;
    ...
    public ParseSave(BufferedReader br) {
    ...
    public void parse() {
        try {
            ...
            while (currentLine.startsWith("Table: ")) {
                numFigure = Integer.parseInt(currentLine.substring(currentLine.indexOf(" ") + 1)); // get the Table
                                                                                                   // number
